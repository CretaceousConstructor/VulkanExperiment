...
{
  public:
	//obsolete,this contor is for tiny obj loader which is not very good
	VkModel(const std::string model_path, VkGraphicsComponent& gfx_);

	VkModel(std::vector<V> &para_vertex, std::vector<InsType> &para_instance_data, std::vector<uint32_t> &para_indices, VkGraphicsComponent& gfx_);

	VkModel(std::vector<V> &para_vertex, std::vector<uint32_t> &para_indices,VkGraphicsComponent& gfx_);
...
//TODO:这个类的资源管理就是个灾难，请速度重构
  private:
	struct Vertices
	{
		...
		void           CleanUp(const VkDeviceManager &para_device_manager);
	};
	struct Indices
	{
		uint32_t       count{};
...
//inline VkModel<V, InsType>::VkModel(const std::string model_path,VkGraphicsComponent& gfx_) :
//    gfx(gfx_),
...
template <class V, class InsType>
inline VkModel<V, InsType>::VkModel(std::vector<V> &para_vertex, std::vector<uint32_t> &para_indices,VkGraphicsComponent& gfx_) :
    gfx(gfx_),
    device_manager(gfx.DeviceMan()),
    window(gfx.Window()),
	command_manager(gfx.CommandMan())
{
	//vertex
	{
		...
		device_manager.CopyBuffer(stagingBuffer, vertices.buffer, bufferSize, *command_manager.GetTransferCommandBuffers().cbegin());
	...
	//index
	{
		...
		device_manager.CreateBufferAndBindToMemo(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indices.buffer, indices.memory, VK_SHARING_MODE_EXCLUSIVE, window.GetSurface());
		//
		device_manager.CopyBuffer(stagingBuffer, indices.buffer, bufferSize, *command_manager.GetTransferCommandBuffers().cbegin());
...
template <class V, class InsType>
VkModel<V, InsType>::VkModel(std::vector<V> &para_vertex, std::vector<InsType> &para_instance_data, std::vector<uint32_t> &para_indices,VkGraphicsComponent& gfx_)
...
template <class V, class InsType>
void VkModel<V, InsType>::Vertices::CleanUp(const VkDeviceManager &para_device_manager)
{
	if (buffer)
	{
...
template <class V, class InsType>
void VkModel<V, InsType>::Indices::CleanUp(const VkDeviceManager &para_device_manager)
{
	if (buffer)
	{
		vkDestroyBuffer(para_device_manager.GetLogicalDevice(), buffer, nullptr);
...
template <class V, class InsType>
void VkModel<V, InsType>::InstancesData::CleanUp(const VkDeviceManager &para_device_manager)
{
