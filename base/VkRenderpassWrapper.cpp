#include "VkRenderpassWrapper.h"
	...
	VkRenderpassWrapper::VkRenderpassWrapper(const std::string &_renderpass_name, std::vector<VkAttachmentDescription> _attachments, std::vector<VkSubpassDependency> _dependencies, std::vector<VkSubpassWrapper> _subpasses, VkDeviceManager &device_manager)
	 :
	    ...
	    render_pass_name(_renderpass_name)
	{
		...
		for (auto &subpass : subpasses)
		{
			...
			{
				color_attachment_refs.emplace_back(attachmentref.slot, attachmentref.layout);
			}
			...
			{
				input_attachment_refs.emplace_back(attachmentref.slot, attachmentref.layout);
			}
			...
			{
				resolve_attachment_refs.emplace_back(attachmentref.slot, attachmentref.layout);
			}
			...
			{
				depth_attachment_refs.emplace_back(attachmentref.slot, attachmentref.layout);
			}
