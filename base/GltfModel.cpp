...

{
	...
	if (fileLoaded)
	{
		...
		LoadImages(glTFInput, load_images_from_file);
...
void GltfModel::LoadImages(tinygltf::Model &input, bool load_from_file_name)
{
	...
	for (size_t i = 0; i < input.images.size(); i++)
	{
		...
		else
		{
			...
			tinygltf::Image &glTFImage      = input.images[i];
			std::string      path           = model_path + "/" + glTFImage.uri;
			...
			{
				images[i].texture.tex_name = glTFImage.name;
				images[i].texture.InitKTXTexture(path, device_manager, window, transfer_command_pool, format_of_image, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
				images[i].texture.InitTextureView(format_of_image, VK_IMAGE_ASPECT_COLOR_BIT);
				images[i].texture.InitSampler();
			}
			else
