...
void VkInitializer::CreateInstance(VkInstance& instance, VkValidationManager &validationManager)
{
	...
	validationFeatureExt.enabledValidationFeatureCount = static_cast<uint32_t>(validationManager.enabled_features.size());
	validationFeatureExt.pEnabledValidationFeatures    = validationManager.enabled_features.data();

	validationFeatureExt.disabledValidationFeatureCount = static_cast<uint32_t>(validationManager.disabled_features.size());
	validationFeatureExt.pDisabledValidationFeatures    = validationManager.disabled_features.data();
	...
	if (VkValidationManager::ValidationLayersEnabled)
	{
		...
		createInfo.enabledLayerCount   = static_cast<uint32_t>(validationManager.required_instance_validation_layers.size());
		createInfo.ppEnabledLayerNames = validationManager.required_instance_validation_layers.data();
