...
RenderGraphV0::VkRsrcUsageInfoRGBuf::VkRsrcUsageInfoRGBuf(std::unique_ptr<VkBufUsageInfoRG> buf_usage_, std::unordered_map<std::string, VirtualResource<VkBufferBase>>::iterator buf_itr_) :
    buf_usage(std::move(buf_usage_)),
    buffer(std::move(buf_itr_))
{
}

RenderGraphV0::VkRsrcUsageInfoRGBuf::VkRsrcUsageInfoRGBuf(VkRsrcUsageInfoRGBuf &&other) :
    buf_usage(std::move(other.buf_usage)), buffer(std::move(other.buffer))
{
}
//
//RenderGraphV0::VkRsrcUsageInfoRGBuf::VkRsrcUsageInfoRGBuf(const VkRsrcUsageInfoRGBuf &other) :
//    buf_usage(std::make_unique<VkBufUsageInfoRG>(*other.buf_usage)), buffer(other.buffer)
//{
//}
