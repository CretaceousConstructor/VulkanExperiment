...
VkSwapChainManager::~VkSwapChainManager()
{
	...
	vkDestroySwapchainKHR(device_manager.GetLogicalDeviceRef(), swap_chain, nullptr);
...
{
	SwapChainSupportDetails swapChainSupport = QuerySwapChainSupport(device_manager.GetPhysicalDeviceRef(), window.GetSurfaceRef());
	////支持的backbuffer格式
	...
	VkDeviceManager::QueueFamilyIndices indices            = VkDeviceManager::FindQueueFamilies(device_manager.GetPhysicalDeviceRef(), window.GetSurface());
	...
	if (vkCreateSwapchainKHR(device_manager.GetLogicalDeviceRef(), &createInfo, nullptr, &swap_chain) != VK_SUCCESS)
	{
		throw std::runtime_error("failed to create swap chain!");
	}

	vkGetSwapchainImagesKHR(device_manager.GetLogicalDeviceRef(), swap_chain, &image_count, nullptr);
	swap_chain_images.resize(image_count);
	vkGetSwapchainImagesKHR(device_manager.GetLogicalDeviceRef(), swap_chain, &image_count, swap_chain_images.data());
	...
	for (size_t i = 0; i < swap_chain_images.size(); i++)
	{
		...
		if (vkCreateImageView(device_manager.GetLogicalDeviceRef(), &createInfo, nullptr, &swap_chain_image_views[i]) != VK_SUCCESS)
		{
...
VkSwapChainManager::SwapChainSupportDetails VkSwapChainManager::QuerySwapChainSupport(const VkPhysicalDevice &physical_device, VkSurfaceKHR &surface)
{
