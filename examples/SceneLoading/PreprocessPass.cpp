...
PreprocessPass::PreprocessPass(VkGraphicsComponent &gfx_, VkRenderpassManager &renderpass_manager_, Global::Resources &common_resources_) :
    VkRenderpassBase(gfx_, renderpass_manager_), pipeline_PP(gfx), device_manager(gfx.DeviceMan()), swapchain_manager(gfx.SwapchainMan()), command_manager(gfx.CommandMan()), global_resources(common_resources_)
{
}
...
void PreprocessPass::ResourceInit()
{
	/**********************************texture***********************************/
	{
		...
		equirectangular_map = texture_factory.ProduceTextureFromImgPath(chiricahua_narrow_path, VK_FORMAT_R32G32B32A32_SFLOAT, sampler_CI, img_view_CI, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
	}
	/**********************************uniform buffer***********************************/
	{
	...
	/*******************************frame buffer images**********************************/
	{
		TextureImgPP texture_img_PP{VK_FORMAT_R16G16B16A16_SFLOAT, extend_of_cube_color_attachment, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT};
		...
		global_resources.cube_mapping = renderpass_manager.GetTextureFactory().ProduceEmptyTexture(texture_img_PP, std::nullopt, img_view_CI);
	}
	...
	/******************************depth buffer images**********************************/
	{
		...
		depth_img_PP.default_image_CI.arrayLayers = 6;
...
void PreprocessPass::CreateLocalCommandBuffers()
{
	/****************************command buffers ************************************/
...
void PreprocessPass::CreateDescriptorSetPools()
{
	/**********************************descriptor pool***********************************/
...
void PreprocessPass::CreateDescriptorSets()
{
	const auto &desciptor_set_factory = renderpass_manager.GetDescriptorSetFactory();
	local_descriptor_set              = desciptor_set_factory.ProduceDescriptorSet(local_descriptor_pool, local_descriptor_set_layout);
}
...
void PreprocessPass::CreateAttachments()
{
	...
	//color_attachments_infos = VkAttachmentInfo::GetAttachmentInfos(color_attachment_info_meme, global_resources.cube_mapping);

	color_attachment_info = VkAttachmentInfo(color_attachment_info_meme, global_resources.cube_mapping);
...
void PreprocessPass::CreateShaders()
{
...
void PreprocessPass::CreateGraphicsPipeline()
{
	return;
}
...
void PreprocessPass::ExecuteRenderpass(const std::vector<VkCommandBuffer> &command_buffers)
{
	...
	global_resources.sky_box->ProcessMaterial(common_layouts, pipeline_PP, pipe_builder, {});

	//RESET COMMAND BUFFER
	vkResetCommandBuffer(local_command_buffer, Vk::NoneFlag);
	...
	//BIND DESCRIPTOR SET
	vkCmdBindDescriptorSets(local_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, global_resources.sky_box->GetPipelineLayout(), 0, Vk::SetCount<1>, &local_descriptor_set, 0, NULL);
	...
	//DRAWING
	global_resources.sky_box->DrawStatically(local_command_buffer);
...
void PreprocessPass::LayoutTransitionStartOfRendering(VkCommandBuffer cmd_buffer, std::optional<size_t> image_index)
{
}
void PreprocessPass::UpdateDescriptorSets()
{
...
void PreprocessPass::LayoutTransitionEndOfRendering(VkCommandBuffer cmd_buffer, std::optional<size_t> image_index)
{
	const auto &color_attachments_tex = global_resources.cube_mapping;
...
void PreprocessPass::UpdateResources(size_t current_image)
{
}
