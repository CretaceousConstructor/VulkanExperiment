...
void RealtimeRenderer::InitRenderpasses()
{
	...
	renderpasses_RG.push_back(std::make_shared<DeferedGeometryPassRGV0>(gfx, renderpass_manager, render_graph_v0, persistent_resources));
...
/********************************************************
 *
 *This function is used by initial version of rendergraph
 *
 ********************************************************
 */
...
//	    .tex_img_PP  = TextureImgPP{DeferedRendering::C_depth_stencil_format, swapchain_manager.GetSwapChainImageExtent(), Vk::ImgCINillFlag},
//	    .img_view_CI = ImgViewCI::PopulateTextureImgViewCI(DeferedRendering::C_depth_stencil_format),
//	    .sample_CI   = std::nullopt,
//	    .bundle_size = Vk::SWAP_IMG_COUNT        //BUNDLE SIZE，很重要的信息or因为是每帧，所以也许可以通过复用，消除这个bundle size的必要性！
//	};
//
...
void RealtimeRenderer::CommandBufferRecording(VkSemaphore general_rendering_finished_semaphore,VkSemaphore image_available_semaphore, size_t img_index)
{
	...
	render_graph_v0.AddGfxPassNode("DeferedCompositionPass", renderpasses_RG[1])
	    ...
	    .Out(
	        ...
	        VkTexture::Descriptor{
	            .tex_img_PP  = TextureImgPP{DeferedRendering::C_depth_stencil_format, swapchain_manager.GetSwapChainImageExtent(), Vk::ImgCINillFlag},
	            .img_view_CI = ImgViewCI::PopulateDepthImgViewCI(DeferedRendering::C_depth_stencil_format),
	            .sample_CI   = std::nullopt,
	        },
	        [](VkTexture::Descriptor &desco) {
