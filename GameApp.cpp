...
//GameApp::GameApp()
//	:
//	vkwindow(800,600,std::string("vulkanApp")),
//	randomEngine(15u),
...
//	vkDestroyDescriptorSetLayout(device, descriptorSetLayoutWrite, nullptr);
//
...
//	pipelineLayoutInfo.pSetLayouts = &descriptorSetLayoutWrite;
//
//
//	pipelineLayoutInfo.pushConstantRangeCount = 0; // Optional
//	pipelineLayoutInfo.pPushConstantRanges = nullptr; // Optional
//
//	if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayoutSubpass0) != VK_SUCCESS) {
...
//
//	pipelineInfo.layout = pipelineLayoutSubpass0;
//
...
//
//
//	auto vertShaderCode1 = readFile("shader0.vert");
//	auto fragShaderCode1 = readFile("shader0.frag");
//
//	VkShaderModule vertShaderModuleSubpass1 = createShaderModule(vertShaderCode1);
//	VkShaderModule fragShaderModuleSubpass1 = createShaderModule(fragShaderCode1);
//
//
//
//
//
//
//	VkPipelineShaderStageCreateInfo vertShaderStageInfo1{};
//	vertShaderStageInfo1.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
//	vertShaderStageInfo1.stage = VK_SHADER_STAGE_VERTEX_BIT;
//	vertShaderStageInfo1.module = vertShaderModuleSubpass1;
//	vertShaderStageInfo1.pName = "main";
//
//
//	VkPipelineShaderStageCreateInfo fragShaderStageInfo1{};
//	fragShaderStageInfo1.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
//	fragShaderStageInfo1.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
//	fragShaderStageInfo1.module = fragShaderModuleSubpass1;
...
//
//
//	VkPipelineShaderStageCreateInfo shaderStages1[] = { vertShaderStageInfo1, fragShaderStageInfo1 };
//
//
//
//
...
//	colorBlending0.pAttachments = &colorBlendAttachment0;
...
//	dynamicState0.dynamicStateCount = 0;
//	dynamicState0.pDynamicStates = VK_NULL_HANDLE;
//
//
//
//
...
//	pipelineLayoutInfo0.pSetLayouts = &descriptorSetLayoutRead;
//
//
//	pipelineLayoutInfo0.pushConstantRangeCount = 0; // Optional
//	pipelineLayoutInfo0.pPushConstantRanges = nullptr; // Optional
//
//	if (vkCreatePipelineLayout(device, &pipelineLayoutInfo0, nullptr, &pipelineLayoutSubpass1) != VK_SUCCESS) {
//		throw std::runtime_error("failed to create pipeline layout!");
//	}
//
//
...
//	depthStencil0.depthWriteEnable = VK_FALSE;
...
//
//
//	VkPipelineInputAssemblyStateCreateInfo inputAssembly0{};
//	inputAssembly0.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
//	inputAssembly0.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
//	inputAssembly0.primitiveRestartEnable = VK_FALSE;
//
//
...
//	pipelineInfo0.pStages = shaderStages1;
//
//	pipelineInfo0.pVertexInputState = &emptyInputStateCI;
//	pipelineInfo0.pInputAssemblyState = &inputAssembly0;
//	pipelineInfo0.pViewportState = &viewportState0;
//
//	pipelineInfo0.pRasterizationState = &rasterizer0;
//
//	pipelineInfo0.pMultisampleState = &multisampling0;
//	pipelineInfo0.pDepthStencilState = &depthStencil0;
//
//
//	pipelineInfo0.pColorBlendState = &colorBlending0;
//
//
//	pipelineInfo0.pDynamicState = nullptr; // Optional
//
//	pipelineInfo0.layout = pipelineLayoutSubpass1;
//
...
//
//
//	vkDestroyShaderModule(device, fragShaderModuleSubpass1, nullptr);
//	vkDestroyShaderModule(device, vertShaderModuleSubpass1, nullptr);
//
//
//
//}
...
//
//
//	std::vector<VkAttachmentReference> WriteColorAttachmentRefsForSubpass0 = { RAttachmentRef,GAttachmentRef };
//
...
//		vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayoutSubpass0, 0, 1, &descriptorSetsWrite[i], 0, NULL);
//
...
//		vkCmdDrawIndexed(commandBuffers[i], indicesGraphics.size(), numOfInstance, 0, 0, 0);
//
//
//
//
...
//		vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayoutSubpass1, 0, 1, &descriptorSetsRead[i], 0, NULL);
//
...
//
//
//	vkWaitForFences(device, 1, &inFlightFences[currentFrame], VK_TRUE, UINT64_MAX); // vkWaitForFences阻塞 查询
...
//{
//	VkDeviceSize bufferSize = sizeof(indicesGraphics[0]) * indicesGraphics.size();
//
//	VkBuffer stagingBuffer;
//	VkDeviceMemory stagingBufferMemory;
//	createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMemory);
...
//	void* data;
//	vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
//	memcpy(data, indicesGraphics.data(), (size_t)bufferSize);
//	vkUnmapMemory(device, stagingBufferMemory);
//
//
//
...
//	*/
//
//	std::vector<VkDescriptorSetLayoutBinding> LayoutBindingWrite;
//
//	LayoutBindingWrite.resize(3);
//	LayoutBindingWrite[0].binding = 0; //index  uniform buffer
//	LayoutBindingWrite[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
//	LayoutBindingWrite[0].descriptorCount = 1;
//	LayoutBindingWrite[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
//	LayoutBindingWrite[0].pImmutableSamplers = nullptr; // Optional
//
//
//
//	LayoutBindingWrite[1].binding = 1; //index  test uniform buffer
//	LayoutBindingWrite[1].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
//	LayoutBindingWrite[1].descriptorCount = 1;
//	LayoutBindingWrite[1].stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
//	LayoutBindingWrite[1].pImmutableSamplers = nullptr; // Optional
//
//
//
//	LayoutBindingWrite[2].binding = 2; //index  texture mapping 
//	LayoutBindingWrite[2].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
//	LayoutBindingWrite[2].descriptorCount = 1;
//	LayoutBindingWrite[2].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
//	LayoutBindingWrite[2].pImmutableSamplers = nullptr; // Optional
//
//
//
//	//LayoutBindingWrite[3].binding = 3; //index  Rcolor
//	//LayoutBindingWrite[3].descriptorCount = 1;
//	//LayoutBindingWrite[3].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
//	//LayoutBindingWrite[3].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
//	//LayoutBindingWrite[3].pImmutableSamplers = nullptr; // Optional
//
//
//	//LayoutBindingWrite[4].binding = 4;//index  Gcolor
//	//LayoutBindingWrite[4].descriptorCount = 1;
//	//LayoutBindingWrite[4].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
//	//LayoutBindingWrite[4].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
//	//LayoutBindingWrite[4].pImmutableSamplers = nullptr; // Optional
//
//
//
//	//LayoutBindingWrite[5].binding = 5;// depth value
//	//LayoutBindingWrite[5].descriptorCount = 1;
//	//LayoutBindingWrite[5].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
//	//LayoutBindingWrite[5].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
//	//LayoutBindingWrite[5].pImmutableSamplers = nullptr; // Optional
//
//
//	VkDescriptorSetLayoutCreateInfo layoutWriteInfo{};
//	layoutWriteInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
//	layoutWriteInfo.bindingCount = (uint32_t)LayoutBindingWrite.size();  //the amount of VkDescriptorSetLayoutBinding
//	layoutWriteInfo.pBindings = LayoutBindingWrite.data();
//
//
//	if (vkCreateDescriptorSetLayout(device, &layoutWriteInfo, nullptr, &descriptorSetLayoutWrite) != VK_SUCCESS) {
...
//	*/
//
//	std::vector<VkDescriptorSetLayoutBinding> LayoutBindingRead;
//	LayoutBindingRead.resize(3);
//
//
//	LayoutBindingRead[0].binding = 0; //index  Rcolor read
//	LayoutBindingRead[0].descriptorCount = 1;
//	LayoutBindingRead[0].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
//	LayoutBindingRead[0].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
//	LayoutBindingRead[0].pImmutableSamplers = nullptr; // Optional
//
//
//	LayoutBindingRead[1].binding = 1;//index  Gcolor read
//	LayoutBindingRead[1].descriptorCount = 1;
//	LayoutBindingRead[1].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
//	LayoutBindingRead[1].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
//	LayoutBindingRead[1].pImmutableSamplers = nullptr; // Optional
//
//
//
//	LayoutBindingRead[2].binding = 2;// depth value read
//	LayoutBindingRead[2].descriptorCount = 1;
//	LayoutBindingRead[2].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
//	LayoutBindingRead[2].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
//	LayoutBindingRead[2].pImmutableSamplers = nullptr; // Optional
//
//
//
//	VkDescriptorSetLayoutCreateInfo layoutReadInfo{};
//	layoutReadInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
//	layoutReadInfo.bindingCount = (uint32_t)LayoutBindingRead.size();  //the amount of VkDescriptorSetLayoutBinding
//	layoutReadInfo.pBindings = LayoutBindingRead.data();
//
//
//	if (vkCreateDescriptorSetLayout(device, &layoutReadInfo, nullptr, &descriptorSetLayoutRead) != VK_SUCCESS) {
//		throw std::runtime_error("failed to create descriptor set layout!");
//	}
//
//
...
//	poolSizes[0].descriptorCount = 9;  // 3+3
//
//
...
//	poolSizes[2].descriptorCount = 9;  //6
//
//
...
//	descriptorPoolCI.maxSets = static_cast<uint32_t>(swapChainImages.size() * 2);
//
//
//
//
...
//
//
//	descriptorSetsWrite.resize(swapChainImages.size());
...
//		allocInfoWrite.descriptorPool = descriptorPool;
//		allocInfoWrite.descriptorSetCount = 1;
//		allocInfoWrite.pSetLayouts = &descriptorSetLayoutWrite;
//
//
//
//		if (vkAllocateDescriptorSets(device, &allocInfoWrite, &descriptorSetsWrite[i]) != VK_SUCCESS) {
...
//
//
//		VkDescriptorBufferInfo bufferInfoUse{};
//		bufferInfo0.buffer = uniformBuffers[i];
//		bufferInfo0.offset = 0;
...
//		imageInfoForSubPass1[2].imageView = depthImageView[i];
...
//
//		writeDescriptorSets[0].pBufferInfo = &bufferInfoUse;
...
//		writeDescriptorSets[2].dstSet = descriptorSetsWrite[i];
...
//		allocInfoRead.descriptorPool = descriptorPool;
//		allocInfoRead.descriptorSetCount = 1;
//		allocInfoRead.pSetLayouts = &descriptorSetLayoutRead;
//
//
//
//		if (vkAllocateDescriptorSets(device, &allocInfoRead, &descriptorSetsRead[i]) != VK_SUCCESS) {
//			throw std::runtime_error("failed to allocate descriptor sets!");
//		}
//
//
...
//		*/
//		writeDescriptorSets[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
//		writeDescriptorSets[0].dstSet = descriptorSetsRead[i];
//		writeDescriptorSets[0].dstBinding = 0;
...
//		*/
//		writeDescriptorSets[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
//		writeDescriptorSets[1].dstSet = descriptorSetsRead[i];
//		writeDescriptorSets[1].dstBinding = 1;
...
//		*/
//		writeDescriptorSets[2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
//		writeDescriptorSets[2].dstSet = descriptorSetsRead[i];
//		writeDescriptorSets[2].dstBinding = 2;
//		writeDescriptorSets[2].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
//		writeDescriptorSets[2].pImageInfo = &imageInfoForSubPass1[2];
//		writeDescriptorSets[2].descriptorCount = 1;
//		writeDescriptorSets[2].dstArrayElement = 0;
//		writeDescriptorSets[2].pBufferInfo = VK_NULL_HANDLE;
//
//
//
//		vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
...
//
//		createImage(swapChainExtent.width, swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage[i], depthImageMemory[i]);
//
//		depthImageView[i] = createImageView(depthImage[i], depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
//
//
//		transitionImageLayout(depthImage[i], depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
//	}
//
//}
//
...
//
//
//		//进行layout transition 从VK_IMAGE_LAYOUT_UNDEFINED 到 VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
