...
float3 calculateNormal(float3 pIn_normal, float3 pIn_tangent, float2 pIn_uv, float handness)
{
    ...
    const float3 sampled_N = textures[NonUniformResourceIndex(1)].Sample(samplerOfTex, pIn_uv).xyz;
    N = mul(TBN, normalize(sampled_N * 2.0 - float3(1.0, 1.0, 1.0)));
...
FSOutput main(VSOutput pIn)
{
    ...
    output.Albedo = textures[NonUniformResourceIndex(0)].Sample(samplerOfTex, pIn.uv);
